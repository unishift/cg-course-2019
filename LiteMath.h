...
static inline float4 clamp(const float4 &u, float a, float b) {
    return make_float4(clamp(u.x, a, b),
                       clamp(u.y, a, b),
                       clamp(u.z, a, b),
                       clamp(u.w, a, b));
}
...
static inline float3 clamp(const float3 &u, float a, float b) {
    return make_float3(clamp(u.x, a, b),
                       clamp(u.y, a, b),
                       clamp(u.z, a, b));
}
...
static inline float2 clamp(const float2 &u, float a, float b) {
    return make_float2(clamp(u.x, a, b),
                       clamp(u.y, a, b));
}
...
static inline float4x4 inverse4x4(float4x4 m1) {
    ...
    //
    float k = 1.0f
        / (m1.row[0].x * m.row[0].x + m1.row[0].y * m.row[1].x + m1.row[0].z * m.row[2].x + m1.row[0].w * m.row[3].x);
...
static inline void _my_frustumf3(float *matrix,
                                 float left,
                                 float right,
                                 float bottom,
                                 float top,
